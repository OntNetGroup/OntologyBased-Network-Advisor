...
public class NOpenEquipmentCloner {
	...
	{					
		...
		for(PElement pElem: elems)
		{
			...
			//if an error occur in this part you can go back to the previous code, 
			//shown below and commented appropriately
			//=============================================================
			...
			/** the dto's type is not based on Concept enumerations
			 *  therefore we needed to change the type int he following cases 
			 */
		...
		for(PLink pLink: links)
		{
			...
			if(isTF(sourceType) && isTF(targetType)){
				...
				FactoryUtil.createInstanceRelation(
					tgtRepository.getBaseModel(), 
					tgtRepository.getNamespace()+sourceId, 
					tgtRepository.getNamespace()+RelationEnum.componentOf.toString(),
					tgtRepository.getNamespace()+targetId
				);
			}
