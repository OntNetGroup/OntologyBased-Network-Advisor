...
//		console.log('Pre Cards: ' + JSON.stringify(preCards));
		if(graph.getElements().length > 16){
...
//					var position = equipPosition;
					var position = {
							x :  equipment.attributes.position.x,
							y :  equipment.attributes.position.y,
					}
...
//									console.log('SET NEW CARD! ' + JSON.stringify(equipment.get('equipment').data.cells[index]));
	...
	hideSubNetwork : function() {
		$('.Subnetwork').hide();
		$('.Layer_Network').hide();
		$('.Matrix').hide();
		$('.handle.e.Layer_Network').show();
		$('.handle.e.Subnetwork').show();
		$('.handle.e.Matrix').show();
	},
	hideLayerNetwork : function() {
		$('.Layer_Network').hide();
		$('.handle.e.Layer_Network').show();
	},
	hideMatrix : function() {
