	...
	start : function(app, subnetworks) {
		...
		var dialog = new joint.ui.Dialog({
			...
			buttons: [
			          ...
			          { action: 'connectAll', content:'Auto-Next', position:'rigth'},
			          ]
		});
		dialog.on('action:next', next);
		dialog.on('action:showImage', showImage);
		dialog.on('action:connectAll', autonext);
		dialog.open();
		...
		function autonext(){
			isAutoNext = true;
			$('.dialog .controls .control-button[data-action="next"]').click();
		};
...
//					owl.addVerticalConnection(tfSource, tfTarget);
			...
			$('.dialog .controls .control-button[data-action="connectAll"]').hide();
...
//				var outputs = owl.getOutputsFromOWL(source.id);
				...
				//if there are ports to connect, connect them
					...
					$('.sourceList #expList').find('li.sourcePort').first().click();
				...
				if(isAutoNext){
					$('.sourceList #expList').find('li.sourcePort').first().click();
					$('.targetList #expList').find('li.targetPort').first().click();
					next();
				};
			}
